...
import gc
from core.model import CSRNet, CrowdRiskClassifier
...
class DensityEstimationCounter:
    def __init__(self, model_path="assets/partB_model_best.pth.tar"):
        ...
        self.model.eval()
        print(f"[OK] CSRNet initialized on {self.device}")
    ...
    def count_people(self, frame):
        try:
            ...
            print(f"[WARNING] CSRNet Error: {e}")
            return 0, None
...
class CrowdMindProcessor:
    def __init__(self):
        ...
        try:
            ...
            print(f"[OK] Risk Model loaded on {self.device}")
            self.has_risk_model = True
        except Exception as e:
            print(f"[WARNING] Risk Model Error: {e}")
